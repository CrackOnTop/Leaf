...
local LeafFps = 60
local LeafBaseFps = 60
local function LeafActiveDelay()
    return 0.035
end
local function LeafIdleDelay()
    return 1
end
local function LeafStream(Enabled)
    if Enabled then
        task.wait(LeafActiveDelay())
        return true
    end
    task.wait(LeafIdleDelay())
    return false
end
local function LeafStreamDelay(Time)
...
local function LeafBF()
    ...
    Library:AddTheme({
        Name = 'LeafLiquid',
        Accent = Color3.fromHex('#8BE8C3'),
        Background = Library:Gradient({
            ['0'] = {Color = Color3.fromHex('#12231E'), Transparency = 0.03},
            ['46'] = {Color = Color3.fromHex('#1E3B32'), Transparency = 0.05},
            ['100'] = {Color = Color3.fromHex('#315F50'), Transparency = 0.08},
        }, {
            Rotation = 28,
        }),
        Outline = Color3.fromHex('#A7F5D3'),
        Text = Color3.fromHex('#F4FFF8'),
        Placeholder = Color3.fromHex('#AFCBC0'),
        Button = Library:Gradient({
            ['0'] = {Color = Color3.fromHex('#203C34'), Transparency = 0.02},
            ['52'] = {Color = Color3.fromHex('#2C5D4D'), Transparency = 0.04},
            ['100'] = {Color = Color3.fromHex('#79DDB6'), Transparency = 0.08},
        }, {
            Rotation = 22,
        }),
        Icon = Color3.fromHex('#B9FFE2'),
    })
    Library:SetTheme('LeafLiquid')
    local Window = Library:CreateWindow({
        ...
        Acrylic = false,
        Background = Library:Gradient({
            ['0'] = {Color = Color3.fromHex('#10231D'), Transparency = 0.08},
            ['38'] = {Color = Color3.fromHex('#22483C'), Transparency = 0.03},
            ['67'] = {Color = Color3.fromHex('#5EC79E'), Transparency = 0.16},
            ['100'] = {Color = Color3.fromHex('#10231D'), Transparency = 0.06},
        }, {
            Rotation = 35,
        }),
        Resizable = false,
        ...
        OpenButton = {
            ...
            Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromHex('#173228')),
                ColorSequenceKeypoint.new(0.45, Color3.fromHex('#63D6A7')),
                ColorSequenceKeypoint.new(0.7, Color3.fromHex('#CFFFF0')),
                ColorSequenceKeypoint.new(1, Color3.fromHex('#245442')),
            }),
        },
    })
    local Colors = {
        Info = Color3.fromHex('#A8D8C1'),
        Green = Color3.fromHex('#7DE8BD'),
        Yellow = Color3.fromHex('#D8C883'),
        Purple = Color3.fromHex('#B8A6D8'),
        Cyan = Color3.fromHex('#8BD6D0'),
        Orange = Color3.fromHex('#D7AA7A'),
        Red = Color3.fromHex('#D98C9A'),
        Blue = Color3.fromHex('#88B8D6'),
        Grey = Color3.fromHex('#9AAEA6'),
    }
    ...
    local TweenProxy = Workspace:FindFirstChild('LeafTweenProxy')
    if TweenProxy then
        TweenProxy:Destroy()
    end
    TweenProxy = Instance.new('Part')
    TweenProxy.Name = 'LeafTweenProxy'
    ...
    local LeafCharacterNoclip = {}
    local LeafBoatNoclip = {}
    local LeafCurrentBoat = nil
    ...
    local function StartCharacterNoclip(Character)
        ...
        if Character then
            ScanNoclipObject(Character, LeafCharacterNoclip)
        end
    end
    local function StopCharacterNoclip()
        RestoreNoclip(LeafCharacterNoclip)
    end
    local function StartBoatNoclip(Boat)
        if not Boat then
            return
        end
        if LeafCurrentBoat ~= Boat then
            RestoreNoclip(LeafBoatNoclip)
            LeafCurrentBoat = Boat
            ScanNoclipObject(Boat, LeafBoatNoclip)
        end
    end
    ...
    local function ResetPhysics(Character)
        ...
        if Root then
            ...
            local BodyClip = Root:FindFirstChild('BodyClip') or Root:FindFirstChild('LeafTweenVelocity') or Root:FindFirstChild('TweenBodyVelocity')
            if BodyClip then
                BodyClip:Destroy()
            end
        end
    ...
    btn.TextStrokeColor3 = Color3.fromRGB(9, 24, 18)
    btn.TextSize = 14
    btn.TextScaled = true
    ...
    task.spawn(function()
        local success, result = pcall(function()
            return game:HttpGet('https://github.com/LeafHubAcademy/LeafHub/raw/refs/heads/main/Code.json')
        end)
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoLevel) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Cake) then continue end
            if _G.Cake then
                pcall(function()
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoNear) then continue end
            if _G.AutoNear then
                pcall(function()
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoBone) then continue end
            if _G.AutoBone then
                pcall(function()
                    local Player = game:GetService('Players').LocalPlayer
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.CastleRaid) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmChest) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoBerry) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.UseSkill) then continue end
            if _G.UseSkill then
                pcall(function()
                    for _, v in pairs(game:GetService('Workspace').Enemies:GetChildren())do
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasDevilFruit and TypeMastery == 'Near Mob') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasDevilFruit and TypeMastery == 'Ecto') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasDevilFruit and TypeMastery == 'Cake') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasDevilFruit and TypeMastery == 'Level') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasDevilFruit and TypeMastery == 'Bone') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.UseSkillGun) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasGun and TypeMastery == 'Near Mob') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasGun and TypeMastery == 'Ecto') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasGun and TypeMastery == 'Cake') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasGun and TypeMastery == 'Level') then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoFarmMasGun and TypeMastery == 'Bone') then continue end
    ...
    task.spawn(function()
        ...
        LocalPlayer = Players.LocalPlayer
        while task.wait() do
            if not LeafStream(_G.AutoFarmRace or _G.AutoTurnV4 or _G.AutoFarmSea or _G.AutoQuestRace or _G.AutoKillTrial or _G.Auto_Dungeon or _G.AttackLeviathan) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoFarmRace) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoRandomBone) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.SpawnCakePrince) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoBoss and _G.SelectBoss) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoMaterial) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            ...
            while task.wait() do
                if not LeafStream(_G.AutoKitsune) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoFindIsland and getIslandConfig(IslandTypeSelected) ~= nil) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoDrive) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoFarmSea) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoElite) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoTpAdvanced) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.TweenToGear) then continue end
    ...
    if Sea1 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Saber and game.Players.LocalPlayer.Data.Level.Value >= 200) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_PoleV1) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Saw) then continue end
                if _G.Auto_Saw then
                    pcall(function()
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Warden) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoHallowSycthe) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoYama) then continue end
                if _G.AutoYama then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoTushita) then continue end
        ...
        local function LeafHasItem(Name)
        ...
        local function LeafItemData(Name)
            local Ok, Inventory = pcall(function()
                return game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('getInventory')
            end)
            if Ok and type(Inventory) == 'table' then
        ...
        local function LeafStartCDK(Type)
            pcall(function()
        ...
        local function LeafNearestEnemy(Names)
        ...
        local function LeafKillCDKEnemy(Enemy, Sword)
        ...
        local function LeafClickPrompt(Object)
        ...
        local function LeafRunYamaTrial(Step)
            if Step == 1 then
                local Enemy = LeafNearestEnemy({'Mythological Pirate'})
                if Enemy then
                    LeafStartCDK('Evil')
                    LeafKillCDKEnemy(Enemy, 'Yama')
                else
            ...
            elseif Step == 2 then
                ...
                if Target then
                    local Enemy = LeafNearestEnemy({Target})
                    if Enemy then
                        LeafKillCDKEnemy(Enemy, 'Yama')
            ...
            elseif Step == 3 then
                if game:GetService('Workspace').Map:FindFirstChild('HellDimension') then
                    local Enemy = LeafNearestEnemy({'Cursed Skeleton', "Hell's Messenger"})
                    if Enemy then
                        LeafKillCDKEnemy(Enemy, 'Yama')
                    else
                ...
                elseif game.Players.LocalPlayer.Backpack:FindFirstChild('Hallow Essence') or game.Players.LocalPlayer.Character:FindFirstChild('Hallow Essence') then
                    ...
                    local Reaper = LeafNearestEnemy({'Soul Reaper'})
                    if Reaper then
        ...
        local function LeafRunTushitaTrial(Step)
            if Step == 1 then
                pcall(function()
            ...
            elseif Step == 2 then
                local Enemy = LeafNearestEnemy({'Pirate Millionaire', 'Pistol Billionaire', 'Forest Pirate', 'Mythological Pirate'}) or LeafNearestEnemy({'Beautiful Pirate', 'Hydra Leader'})
                if Enemy then
                    LeafKillCDKEnemy(Enemy, 'Tushita')
                else
                    Tween(CFrame.new(-5569.12939, 320.95163, -3037.64575))
                end
            elseif Step == 3 then
                if game:GetService('Workspace').Map:FindFirstChild('HeavenlyDimension') then
                    local Enemy = LeafNearestEnemy({'Cursed Skeleton', "Heaven's Guardian"})
                    if Enemy then
                        LeafKillCDKEnemy(Enemy, 'Tushita')
                    else
                        for _, Name in ipairs({'Torch1', 'Torch2', 'Torch3'}) do
                    ...
                    local Queen = LeafNearestEnemy({'Cake Queen'})
                    if Queen then
                        LeafKillCDKEnemy(Queen, 'Tushita')
                    else
        ...
        local function LeafRunCDKBoss()
            local Boss = LeafNearestEnemy({'Cursed Skeleton Boss'})
            if Boss then
                LeafKillCDKEnemy(Boss, 'Tushita')
            else
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoQuestCDK) then continue end
                pcall(function()
                    if LeafHasItem('Cursed Dual Katana') then
                        _G.AutoBone = false
                        return
                    end
                    if not LeafHasItem('Yama') or not LeafHasItem('Tushita') then
                        return
                    end
                    if LeafMaterial('Alucard Fragment') < 6 then
                        if LeafMastery('Yama') < 350 then
                            _G.AutoBone = true
                            LeafLoadSword('Yama')
                            local Enemy = LeafNearestEnemy({'Reborn Skeleton', 'Living Zombie', 'Demonic Soul', 'Posessed Mummy'})
                            if Enemy then
                                LeafKillCDKEnemy(Enemy, 'Yama')
                            else
                        ...
                        if LeafMastery('Tushita') < 350 then
                            _G.AutoBone = true
                            LeafLoadSword('Tushita')
                            local Enemy = LeafNearestEnemy({'Reborn Skeleton', 'Living Zombie', 'Demonic Soul', 'Posessed Mummy'})
                            if Enemy then
                                LeafKillCDKEnemy(Enemy, 'Tushita')
                            else
                        ...
                        _G.AutoBone = false
                        local Evil = LeafCDKProgress('Evil')
                        if Evil ~= -2 and Evil ~= 4 then
                            LeafStartCDK('Evil')
                            LeafRunYamaTrial(Evil == 1 and 2 or Evil == 2 and 3 or 1)
                            return
                        end
                        local Good = LeafCDKProgress('Good')
                        if Good ~= -2 and Good ~= 3 then
                            LeafStartCDK('Good')
                            LeafRunTushitaTrial(Good == 1 and 2 or Good == 2 and 3 or 1)
                            return
                        end
                        return
                    end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Holy_Torch) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Canvander) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Buddy) then continue end
        ...
        task.spawn(function()
            pcall(function()
                while task.wait() do
                    if not LeafStream(_G.Auto_MusketeerHat) then continue end
        ...
        task.spawn(function()
            ...
            while task.wait() do
                if not LeafStream(_G.Auto_SkullGuitar) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            pcall(function()
                while task.wait() do
                    if not LeafStream(_G.Auto_RainbowHaki) then continue end
    ...
    if Sea2 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Factory) then continue end
                if _G.Factory then
        ...
        task.spawn(function()
            pcall(function()
                while task.wait() do
                    if not LeafStream(_G.AutoFarmSwan) then continue end
        ...
        task.spawn(function()
            pcall(function()
                while task.wait() do
                    if not LeafStream(_G.Auto_Regoku) then continue end
    ...
    if Sea2 then
        ...
        task.spawn(function()
            pcall(function()
                while task.wait() do
                    if not LeafStream(_G.AutoEvoRace) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(FarmPos and MonFarm) then continue end
            if FarmPos and MonFarm then
                pcall(function()
                    BringMob()
                end)
                LeafStreamDelay(0.15)
            end
        end
    end)
    Tabs.Misc:Toggle({
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Auto_Stats_Devil_Fruit) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Auto_Stats_Gun) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Auto_Stats_Sword) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Auto_Stats_Defense) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Auto_Stats_Melee) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.TeleportPly) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.WalkonWater and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild('HumanoidRootPart')) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Aimbot_Camera and _G.TargetPlayer and _G.TargetPlayer.Character and _G.TargetPlayer.Character:FindFirstChild('HumanoidRootPart')) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Silent_Aim and _G.TargetPlayer and _G.TargetPlayer.Character and _G.TargetPlayer.Character:FindFirstChild('HumanoidRootPart')) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Aimbot_Gun and _G.TargetPlayer and _G.TargetPlayer.Character and _G.TargetPlayer.Character:FindFirstChild('HumanoidRootPart')) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.EspPlayer) then continue end
    ...
    if Sea2 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Sea3) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.AutoStoreFruit) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Random_Auto) then continue end
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.Tweenfruit) then continue end
    ...
    if Sea2 or Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Buy_Chips_Dungeon) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_StartRaid and (Sea2 or Sea3)) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(AutoAwakenAbilities) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Autofruit) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Law) then continue end
                if _G.Auto_Law then
                    pcall(function()
    ...
    if Dungeon then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_RandomTrinket) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Start) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Dungeon and _G.SelectedBuffs and #_G.SelectedBuffs > 0) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.Auto_Dungeon) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoUpgrade) then continue end
    ...
    local JobInputValue = ''
    local ServerHistoryPath = 'Leaf Hub/assets/Servers.json'
    local ServerHistory = {}
    pcall(function()
    ...
    local function SaveServerId(ServerId)
        ...
        pcall(function()
            if makefolder and not isfolder('Leaf Hub') then
                makefolder('Leaf Hub')
            end
            if makefolder and not isfolder('Leaf Hub/assets') then
                makefolder('Leaf Hub/assets')
            end
            if writefile then
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.TweenToFrozenDimension) then continue end
    ...
    if Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoFarmBlazeEmber) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(isQuestCompleated()) then continue end
        ...
        task.spawn(function()
            ...
            while task.wait() do
                if not LeafStream(_G.TweenToPrehistoric) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoVolcanic) then continue end
        ...
        Tabs.Sea:Toggle({
            ...
            Callback = function(v)
                ...
                task.spawn(function()
                    while task.wait() do
                        if not LeafStream(_G.AttackLeviathan) then continue end
                        if _G.AttackLeviathan then
                            local player = game.Players.LocalPlayer
                            local character = player.Character
                            local hrp = character and character:FindFirstChild('HumanoidRootPart')
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoShootHeart) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoRepairShip) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoWoodenPlank) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoWoodenPlank and CurrentTarget) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoIgnite) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoCollectToggle) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoTyrant) then continue end
    ...
    if Sea1 or Sea2 or Sea3 then
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoCraftBait) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoSellFish) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoAcceptQuest) then continue end
        ...
        task.spawn(function()
            while task.wait() do
                if not LeafStream(_G.AutoFishing) then continue end
...
local function LeafDoor()
    ...
    Library:AddTheme({
        ...
        Accent = Color3.fromHex('#8BE8C3'),
        Background = Library:Gradient({
            ['0'] = {Color = Color3.fromHex('#121D1A'), Transparency = 0.03},
            ['45'] = {Color = Color3.fromHex('#1D342E'), Transparency = 0.05},
            ['100'] = {Color = Color3.fromHex('#2D5A4D'), Transparency = 0.08},
        }, {
            Rotation = 28,
        }),
        Outline = Color3.fromHex('#A7F5D3'),
    ...
    Library:SetTheme('LeafDoorLiquid')
    local Window = Library:CreateWindow({
        Title = 'Leaf Hub',
        Icon = 'solar:leaf-bold',
        Folder = 'Leaf Hub',
    ...
    local EspFolder = EspParent:FindFirstChild('LeafDoorEspFolder')
    if EspFolder then
        EspFolder:Destroy()
    end
    EspFolder = Instance.new('Folder')
    EspFolder.Name = 'LeafDoorEspFolder'
    EspFolder.Parent = EspParent
    local EspId = 0
    local EspMark = 0
    local DoorWalkSpeed = 16
    local LeafDoorRenderDistance = 420
    ...
    local function GetObjectId(Object)
        ...
        local Id = Object:GetAttribute('LeafDoorEspId')
        if Id then
            return tostring(Id)
        end
        EspId = EspId + 1
        Id = tostring(EspId)
        pcall(function()
            Object:SetAttribute('LeafDoorEspId', Id)
        end)
        return Id
    end
    ...
    local function CreateEspGui(Name, Color, Group)
        ...
        Gui.MaxDistance = LeafDoorRenderDistance
    ...
    local function EnsureEspAura(Group, Id, Object, Color, Mark)
        ...
        local AuraName = 'LeafDoorAura_' .. Group .. '_' .. Id
    ...
    local function EnsureEsp(Group, Object, Text, Color, Mark)
        ...
        local Root = GetLocalRoot()
        if Root and (Root.Position - Part.Position).Magnitude > LeafDoorRenderDistance then
            return
        end
        local Id = GetObjectId(Object)
        local GuiName = 'LeafDoorEsp_' .. Group .. '_' .. Id
    ...
    local function AddEntityNotify(Object)
        if not Object or Object:GetAttribute('LeafEntitySeen') then
            return
        end
        pcall(function()
            Library:Notify({
        ...
        pcall(function()
            Object:SetAttribute('LeafEntitySeen', true)
        end)
    end
    ...
    Tabs.General:Slider({
        ...
        Callback = function(Value)
            ...
            _G.LeafDoorWalkSpeed = DoorWalkSpeed
            SetWalkSpeed()
        end,
    })
    Tabs.General:Button({
    ...
    task.spawn(function()
        while task.wait() do
            local AnyEsp = _G.LeafDoorEspDoor or _G.LeafDoorEspHiding or _G.LeafDoorEspGold or _G.LeafDoorEspItem or _G.LeafDoorEspPlayer
            if not LeafStream(AnyEsp) then
                continue
            end
            pcall(function()
                if _G.LeafDoorEspDoor then
                    AddDoorEsp()
                end
                if _G.LeafDoorEspHiding then
                    AddHidingEsp()
                end
                if _G.LeafDoorEspGold then
                    AddGoldEsp()
                end
                if _G.LeafDoorEspItem then
                    AddItemEsp()
                end
                if _G.LeafDoorEspPlayer then
                    AddPlayerEsp()
                end
                AddEntityEsp()
            end)
            LeafStreamDelay(1.25)
        end
    end)
    task.spawn(function()
        while task.wait() do
...
local function LeafSailorPiece()
    local Players = game:GetService('Players')
    local ReplicatedStorage = game:GetService('ReplicatedStorage')
    ...
    local function GetTweenProxy(Root)
        ...
        TweenProxy = Instance.new('Part')
        TweenProxy.Name = 'LeafSailorTweenProxy'
        TweenProxy.Anchored = true
        TweenProxy.CanCollide = false
        TweenProxy.CanTouch = false
    ...
    local function UseSkill(Key)
        local SkillKey = tostring(Key or ''):upper()
        local Slot = ({Z = 1, X = 2, C = 3, V = 4, F = 5})[SkillKey]
        if not Slot then
            return
        end
        local Tool = GetEquippedTool()
        local ToolName = Tool and Tool.Name or ''
        if GetToolTypeFromModule(ToolName) == 'Power' then
            local KeyCode = Enum.KeyCode[SkillKey]
            if KeyCode then
                SafeFire(Remotes.UseFruit, 'UseAbility', {FruitPower = ToolName:gsub(' Fruit', ''), KeyCode = KeyCode})
            end
        else
            SafeFire(Remotes.UseSkill, Slot)
        end
    end
    local function Attack(Target)
        if not Target or not Alive(Target) then
            local Root = GetRoot()
            local Best, BestDistance = nil, 80
            if Root and PATH.Mobs then
                for _, Npc in ipairs(PATH.Mobs:GetChildren()) do
        ...
        local Now = tick()
        EquipWeapon()
        if Now - LastM1 >= 0.08 then
            local TargetCF = Pivot(Target)
            if TargetCF then
                SafeFire(Remotes.M1, TargetCF.Position)
            else
                SafeFire(Remotes.M1)
            end
            LastM1 = Now
        end
        if _G.LeafSailorAutoSkill and Now - LastSkill >= 0.35 then
    ...
    local function GetBestQuestNPC()
        ...
        for NpcId, QuestData in pairs(Repeatable) do
            local Required = tonumber(QuestData.recommendedLevel) or 0
            if Level >= Required and Required > Highest then
                BestNPC = NpcId
                Highest = Required
            end
        end
    ...
    local function UpdateQuest()
        local QuestNPC = GetBestQuestNPC()
        local QuestGui = PlayerGui:FindFirstChild('QuestUI') and PlayerGui.QuestUI:FindFirstChild('Quest')
        if not QuestGui then
            SafeFire(Remotes.QuestAccept, QuestNPC)
            LastTargetName = QuestNPC
            return QuestNPC
        end
        if LastTargetName ~= QuestNPC or not QuestGui.Visible then
            SafeFire(Remotes.QuestAbandon, 'repeatable')
            local AbandonTick = tick()
            while QuestGui.Visible and tick() - AbandonTick < 3 do
                task.wait(0.15)
            end
            SafeFire(Remotes.QuestAccept, QuestNPC)
            local AcceptTick = tick()
            while not QuestGui.Visible and tick() - AcceptTick < 4 do
                task.wait(0.15)
                if math.floor((tick() - AcceptTick) * 10) % 8 == 0 then
                    SafeFire(Remotes.QuestAccept, QuestNPC)
                end
            end
    ...
    local function GetQuestMobName(QuestNPC)
        ...
        if not QuestData or type(QuestData.requirements) ~= 'table' then
            return nil
        end
        for _, Requirement in ipairs(QuestData.requirements) do
            if type(Requirement) == 'table' and Requirement.npcType then
                return CleanName(Requirement.npcType):lower()
            end
        end
        return nil
    end
    local function FindBestLevelMob()
        local QuestNPC = UpdateQuest()
        local TargetMobType = GetQuestMobName(QuestNPC)
        if not TargetMobType then
            return nil
        end
        return GetBestMobCluster({[TargetMobType] = true})
    end
    local function AcceptQuestForTarget(Target)
        if _G.LeafSailorLevelFarm then
            UpdateQuest()
        end
    end
    ...
    Tabs.General:Dropdown({
        Title = 'Farm Type',
        Values = Tables.FarmType,
        Value = 'Behind',
        Multi = false,
        Callback = function(Value)
            SelectedFarmType = Value or 'Behind'
        end,
    })
    Tabs.General:Slider({
        Title = 'Distance',
    ...
    Tabs.General:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAllMobFarm = Value
        end,
    })
    Tabs.General:Dropdown({
        Title = 'Select Skills',
        Values = Tables.Skills,
        Value = {'Z', 'X', 'C', 'V'},
        Multi = true,
        Callback = function(Value)
            ...
            if type(Value) == 'table' then
                for Key, Enabled in pairs(Value) do
                    if Enabled == true then
                        SelectedSkills[#SelectedSkills + 1] = tostring(Key):upper()
                    elseif type(Key) == 'number' then
                        SelectedSkills[#SelectedSkills + 1] = tostring(Enabled):upper()
                    end
                end
            end
        end,
    ...
    Tabs.General:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorObservationHaki = Value
        end,
    })
    Tabs.General:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorConquerorHaki = Value
        end,
    })
    ...
    Tabs.Boss:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAllBossesFarm = Value
        end,
    })
    Tabs.Boss:Dropdown({
        Title = 'Select Summon',
    ...
    Tabs.Boss:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorSummonBossFarm = Value
        end,
    })
    Tabs.Dungeon:Dropdown({
    ...
    Tabs.Dungeon:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAutoDungeon = Value
        end,
    })
    Tabs.Dungeon:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAutoInfiniteTower = Value
        end,
    })
    Tabs.Dungeon:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorDungeonAutofarm = Value
        end,
    })
    Tabs.Stats:Dropdown({
        Title = 'Select Stats',
    ...
    Tabs.Inventory:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAutoCraftItem = Value
        end,
    })
    Tabs.Inventory:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorArtifactMilestone = Value
        end,
    })
    Tabs.Inventory:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorArtifactUpgrade = Value
        end,
    })
    Tabs.Inventory:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorArtifactEquip = Value
        end,
    })
    ...
    Tabs.Shop:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAutoMerchant = Value
        end,
    })
    Tabs.Shop:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAutoDungeonMerchant = Value
        end,
    })
    Tabs.Shop:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAutoValentineMerchant = Value
        end,
    })
    Tabs.Shop:Toggle({
        ...
        Value = false,
        Callback = function(Value)
            _G.LeafSailorAutoTowerMerchant = Value
        end,
    })
    ...
    task.spawn(function()
        while task.wait() do
            if not _G.LeafSailorArmHaki and not _G.LeafSailorObservationHaki and not _G.LeafSailorConquerorHaki then
                LeafStreamDelay(0.8)
                continue
            end
            if _G.LeafSailorArmHaki then
                SafeFire(Remotes.ArmHaki)
            end
            if _G.LeafSailorObservationHaki then
                SafeFire(Remotes.ObserHaki)
            end
            if _G.LeafSailorConquerorHaki then
                SafeFire(Remotes.ConquerorHaki, 'Activate')
            end
            LeafStreamDelay(0.75)
        end
    end)
    task.spawn(function()
        while task.wait() do
            local Enabled = _G.LeafSailorLevelFarm or _G.LeafSailorMobFarm or _G.LeafSailorAllMobFarm
            if not LeafStream(Enabled) then continue end
            local Target
            if _G.LeafSailorLevelFarm then
                Target = FindBestLevelMob()
                MoveAndKill(Target, true)
            elseif _G.LeafSailorMobFarm then
                Target = FindNearestByName(SelectedMob, false)
                MoveAndKill(Target, false)
            elseif _G.LeafSailorAllMobFarm then
                UpdateNPCLists()
    ...
    task.spawn(function()
        while task.wait() do
            local Enabled = _G.LeafSailorBossesFarm or _G.LeafSailorAllBossesFarm or _G.LeafSailorSummonBossFarm
            if not LeafStream(Enabled) then continue end
            local Target
            if _G.LeafSailorBossesFarm then
                Target = FindNearestByName(SelectedBoss, true)
            elseif _G.LeafSailorAllBossesFarm or _G.LeafSailorSummonBossFarm then
                UpdateNPCLists()
            ...
            if not Target and (_G.LeafSailorAutoSummon or _G.LeafSailorSummonBossFarm) then
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.LeafSailorAutoStats) then continue end
            local Points = 0
            pcall(function()
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.LeafSailorAutoAscend or _G.LeafSailorAutoTrait or _G.LeafSailorAutoRace or _G.LeafSailorAutoClan) then continue end
            if _G.LeafSailorAutoAscend then
                SafeFire(Remotes.Ascend)
                SafeFire(Remotes.CloseAscend)
            end
            if _G.LeafSailorAutoTrait then
                SafeFire(Remotes.TraitConfirm, true)
                SafeFire(Remotes.RollTrait)
            end
            if _G.LeafSailorAutoRace then
                SafeFire(Remotes.UseItem, 'Use', 'Race Reroll', 1)
            end
            if _G.LeafSailorAutoClan then
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.LeafSailorAutoDungeon or _G.LeafSailorAutoInfiniteTower or _G.LeafSailorDungeonAutofarm) then continue end
            if tick() - LastDungeon > 12 then
                if _G.LeafSailorAutoInfiniteTower then
            ...
            if _G.LeafSailorDungeonAutofarm then
                local Target
                if PATH.Mobs then
                    for _, Npc in ipairs(PATH.Mobs:GetChildren()) do
                        if Npc:IsA('Model') and Alive(Npc) then
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.LeafSailorAutoMerchant or _G.LeafSailorAutoDungeonMerchant or _G.LeafSailorAutoValentineMerchant or _G.LeafSailorAutoTowerMerchant) then continue end
            ...
            for Item, Enabled in pairs(SelectedMerchantItems) do
                ...
                if type(Name) == 'string' and Name ~= '' then
                    if _G.LeafSailorAutoValentineMerchant then
    ...
    task.spawn(function()
        while task.wait() do
            if not LeafStream(_G.LeafSailorAutoCraftItem or _G.LeafSailorArtifactMilestone or _G.LeafSailorArtifactUpgrade or _G.LeafSailorArtifactEquip) then continue end
            if _G.LeafSailorAutoCraftItem then
            ...
            if _G.LeafSailorArtifactMilestone then
                for I = 1, 40 do
                    SafeFire(Remotes.ArtifactClaim, I)
                end
            end
            if _G.LeafSailorArtifactUpgrade then
                SafeFire(Remotes.ArtifactMassUpgrade)
            end
            if _G.LeafSailorArtifactEquip then
...
local function LeafKingLegacy()
    ...
    local SelectedSkills = {'Z', 'X', 'C', 'V'}
    local Flags = {}
    local CurrentTween, TweenProxy, TweenFollow, LastTweenPosition, LastTweenTick
    local QuestCache, QuestTick, LastM1, LastSkill = {}, 0, 0, 0
    local EventChestCache = {}
    local MaterialTargets = {
        ['Log'] = {'Tree'},
        ['Pile of Bones'] = {'Skull Pirate [Lv. 3050]', 'Elite Skeleton [Lv. 3100]'},
        ['Fresh Fish'] = {'Karate Fishman [Lv. 200]', 'Fighter Fishman [Lv. 180]', 'Shark Man [Lv. 230]'},
        ["Angellic's Feather"] = {'Sky Soldier [Lv. 800]', 'Ball Man [Lv. 850]'},
        ["Sea King's Blood"] = {'SeaKing', 'HydraSeaKing'},
        ['Candy'] = {'Jack o lantern [Lv. 10000]'},
    }
    local BossTargets = {
        ['Minion Event'] = {'Minion', 'Boss'},
        ['Sea King'] = {'SeaKing'},
        ['Hydra Sea King'] = {'HydraSeaKing'},
        ['Ghost Ship'] = {'Ghost Ship'},
        ['Abyssal Tyrant'] = {'SeaDragon'},
        ['Chaos Kraken'] = {'FuryTentacle'},
        ['Deepsea Crusher'] = {'ThirdSeaEldritch Crab'},
        ['Third Sea Dragon'] = {'ThirdSeaDragon'},
        ['Kaido'] = {'Dragon [Lv. 5000]', 'Elite Skeleton [Lv. 3100]'},
        ['Expert Swordman'] = {'Expert Swordman [Lv. 3000]'},
        ['Bushido Ape'] = {'Bushido Ape [Lv. 5000]'},
        ['Lord Saber'] = {'Lord of Saber [Lv. 8500]'},
        ['Jack O Lantern'] = {'Jack o lantern [Lv. 10000]'},
        ['Skull King'] = {'Skull King [Lv. 5000]', 'Skull Pirate [Lv. 3050]'},
        ['Gorilla King'] = {'Gorilla King [Lv. 1000]', 'Gorilla [Lv. 900]'},
        ['Big Mom'] = {'Big Mom [Lv. 5000]'},
        ['King Samurai'] = {'King Samurai [Lv. 3500]'},
        ['Quake Woman'] = {'Quake Woman [Lv. 1925]'},
    }
    local WindUI = loadstring(game:HttpGet('https://github.com/Footagesus/WindUI/releases/latest/download/main.lua'))()
    local Library = WindUI
    pcall(function()
    ...
    local function TabOptions(Title, Icon, Color)
        return {Title = Title, Icon = Icon, IconColor = Color, IconShape = 'Square', Border = true}
    end
    local Tabs = {
    ...
    Tabs.Info:Paragraph({Title = 'Info', Desc = "Press 'G' to toggle UI.", Color = 'Grey', Image = 'rbxassetid://13858820419', ImageSize = 22})
    Tabs.Info:Paragraph({Title = 'Support Game', Desc = '[+] Blox Fruits\n[+] Doors\n[+] Sailor Piece\n[+] King Legacy', Color = 'Green', Image = 'rbxassetid://13858820419', ImageSize = 22})
    pcall(function()
    ...
    local function Character()
        ...
        return Char and Char:FindFirstChild('HumanoidRootPart') and Char:FindFirstChildOfClass('Humanoid') and Char or nil
    end
    local function StopTween()
        if CurrentTween then CurrentTween:Cancel() CurrentTween = nil end
        if TweenFollow then TweenFollow:Disconnect() TweenFollow = nil end
        LastTweenPosition = nil
        local Char = Character()
        if Char then
            Char.HumanoidRootPart.Velocity = Vector3.zero
            Char.HumanoidRootPart.AssemblyLinearVelocity = Vector3.zero
            local BodyClip = Char.HumanoidRootPart:FindFirstChild('LeafKingBodyClip')
    ...
    local function Tween(Target)
        local TargetCF, Char = ToCF(Target), Character()
        if not TargetCF or not Char or Char.Humanoid.Health <= 0 then return end
        if LastTweenPosition and (TargetCF.Position - LastTweenPosition).Magnitude <= 8 and tick() - LastTweenTick <= 0.25 then return end
        if not TweenProxy or not TweenProxy.Parent then
            TweenProxy = Instance.new('Part')
            TweenProxy.Name = 'LeafKingTweenProxy'
            TweenProxy.Size = Vector3.new(1, 1, 1)
            TweenProxy.Transparency = 1
            TweenProxy.Anchored = true
            TweenProxy.CanCollide = false
        ...
        BodyClip.Velocity = Vector3.zero
        BodyClip.Parent = Root
        if not TweenFollow then
            TweenFollow = RunService.Heartbeat:Connect(function()
                local Now = Character()
                if not CurrentTween or CurrentTween.PlaybackState ~= Enum.PlaybackState.Playing or not Now then
                    if TweenFollow then TweenFollow:Disconnect() TweenFollow = nil end
                    return
                end
                Now.HumanoidRootPart.CFrame = TweenProxy.CFrame
                Now.HumanoidRootPart.Velocity = Vector3.zero
                Now.HumanoidRootPart.AssemblyLinearVelocity = Vector3.zero
                Now.Humanoid.Sit = false
                Now.Humanoid:ChangeState(11)
            end)
        end
        LastTweenPosition = TargetCF.Position
        LastTweenTick = tick()
        CurrentTween = TweenService:Create(TweenProxy, TweenInfo.new(math.max(Distance / TweenSpeed, 0.05), Enum.EasingStyle.Linear), {CFrame = TargetCF})
        CurrentTween:Play()
        CurrentTween.Completed:Once(function(State)
            if State == Enum.PlaybackState.Completed then
                ...
                local Now = Character()
                if Now then
                    Now.HumanoidRootPart.CFrame = TargetCF
                    if TweenProxy then TweenProxy.CFrame = TargetCF end
                end
            end
        end)
    end
    local function ToolPrefix(Tool)
        if not Tool then return 'FS' end
        if Tool.ToolTip == 'Sword' then return 'SW' end
        if Tool.ToolTip == 'Fruit Power' then return 'FP' end
        return 'FS'
    end
    local function GetWeapon()
        local Map = {Melee = {'Combat', 'Melee'}, Sword = {'Sword'}, Fruit = {'Fruit Power'}}
        for _, Container in ipairs({LocalPlayer.Character, LocalPlayer:FindFirstChild('Backpack')}) do
            if Container then
                for _, Tool in ipairs(Container:GetChildren()) do
                    if Tool:IsA('Tool') then
                        for _, Name in ipairs(Map[SelectedWeapon] or Map.Melee) do
                            if Tool.ToolTip == Name or Tool.Name == Name or Tool.ToolTip:find(Name) then
                                return Tool
                            end
                        end
                    end
    ...
    local function EquipWeapon()
        local Char, Tool = Character(), GetWeapon()
        if Char and Tool and Tool.Parent ~= Char then pcall(Char.Humanoid.EquipTool, Char.Humanoid, Tool) end
        return Tool or (Char and Char:FindFirstChildOfClass('Tool'))
    end
    local function SkillReady(Key)
        local Tool = EquipWeapon()
        local Folder = Tool and Tool.ToolTip == 'Sword' and 'SWFrame' or Tool and Tool.ToolTip == 'Fruit Power' and 'DFFrame' or 'FSFrame'
        local Frame = LocalPlayer.PlayerGui:FindFirstChild('SkillCooldown') and LocalPlayer.PlayerGui.SkillCooldown:FindFirstChild(Folder)
        local Slot = Frame and Frame:FindFirstChild(Key)
        return not Slot or not Slot:FindFirstChild('Locked') or (not Slot.Locked.Visible and Slot.Frame.Frame.AbsoluteSize.X > 0)
    end
    local function Attack()
        local Tool = EquipWeapon()
        if not Tool then return end
        if tick() - LastM1 >= 0.08 then
            pcall(function()
                ReplicatedStorage.Chest.Remotes.Functions.SkillAction:InvokeServer(ToolPrefix(Tool) .. '_' .. Tool.Name .. '_M1')
            end)
            LastM1 = tick()
        end
        if tick() - LastSkill >= 0.35 then
    ...
    local function GetMaterial(Name)
        local Ok, Data = pcall(function()
            return game:GetService('HttpService'):JSONDecode(LocalPlayer.PlayerStats.Material.Value)
        end)
        if Ok and type(Data) == 'table' then
            return tonumber(Data[Name]) or 0
        end
        return 0
    end
    local function AllNpc()
        return Workspace:FindFirstChild('AllNPC') and Workspace.AllNPC:GetChildren() or {}
    end
    local function MonsterFolders()
        return {Workspace:FindFirstChild('Monster') and Workspace.Monster:FindFirstChild('Boss'), Workspace:FindFirstChild('Monster') and Workspace.Monster:FindFirstChild('Mon'), Workspace:FindFirstChild('SeaMonster'), Workspace:FindFirstChild('GhostMonster'), ReplicatedStorage:FindFirstChild('MOB')}
    end
    local function Alive(Model)
        ...
        for _, Folder in ipairs(MonsterFolders()) do
            if Folder then
                for _, Mob in ipairs(Folder:GetChildren()) do
                    if Alive(Mob) and table.find(Names, Mob.Name) then return Mob end
                end
            end
        end
    end
    local function FindNearestMob()
        local Char, Best, BestDistance = Character(), nil, math.huge
    ...
    local function QuestData()
        if tick() - QuestTick <= 3 and QuestCache[1] then return QuestCache[1] end
        QuestCache = {}
        local Ok, Manager = pcall(function() return require(ReplicatedStorage.Chest.Modules.QuestManager) end)
        if Ok and Manager then
            for Title, Data in pairs(Manager) do
                if type(Data) == 'table' and not Data.DailyQuest and Data.Mob and Data.Level and Data.Mob:find('Lv') and Data.Level <= LocalPlayer.PlayerStats.lvl.Value then
                    table.insert(QuestCache, {Title = Title, Mob = Data.Mob, Level = Data.Level})
                end
            end
        end
        table.sort(QuestCache, function(A, B) return A.Level > B.Level end)
        local Quest = QuestCache[1]
        if Quest then
            local BestNpc
            for _, NPC in ipairs(AllNpc()) do
                local LevelMin = NPC:GetAttribute('LevelMin')
                if LevelMin and LevelMin <= Quest.Level and (not BestNpc or LevelMin > BestNpc.Level) then
                    BestNpc = {Level = LevelMin, CFrame = NPC.CFrame}
                end
            end
            Quest.NPCCFrame = BestNpc and BestNpc.CFrame or nil
            if Sea1 and Quest.Level >= 2200 and LocalPlayer.PlayerStats.lvl.Value >= 2250 then
                Quest.Mob, Quest.Title, Quest.Level = 'Seasoned Fishman [Lv. 2200]', 'Kill 1 Seasoned Fishman', 2200
            elseif Sea2 and LocalPlayer.PlayerStats.lvl.Value >= 3950 then
                Quest.Mob, Quest.Title, Quest.Level = 'Ryu [Lv. 3975]', 'Kill 1 Ryu', 3950
            end
        end
        QuestTick = tick()
        return Quest
    ...
    local function AcceptQuest(Quest)
        if Quest and Quest.NPCCFrame then
            Tween(Quest.NPCCFrame * CFrame.new(0, 0, -3))
            pcall(function()
                ReplicatedStorage.Chest.Remotes.Functions.Quest:InvokeServer('take', Quest.Title)
            end)
        end
    end
    local function LoadStoredMob(Name)
        local Stored = ReplicatedStorage:FindFirstChild('MOB') and ReplicatedStorage.MOB:FindFirstChild(Name)
        if Stored then
            Tween(Stored:GetPivot() * CFrame.new(0, 80, 0))
            return true
        end
        return false
    end
    local function FarmNamed(Names)
        local Mob = FindMob(Names)
        if Mob then
            return MoveAttack(Mob)
        end
        for _, Name in ipairs(Names) do
            if LoadStoredMob(Name) then return true end
        end
        return false
    end
    local function TouchNpc(Name)
        local NPC = Workspace:FindFirstChild('AllNPC') and Workspace.AllNPC:FindFirstChild(Name)
        if NPC then
    ...
    local function CollectSeaChest()
        for _, Island in ipairs(Workspace:FindFirstChild('Island') and Workspace.Island:GetChildren() or {}) do
            if Island.Name:find('Legacy Island') and Island:FindFirstChild('ChestSpawner') then
                Tween(Island.ChestSpawner.CFrame)
                return true
            elseif Island.Name:find('Sea King') and Island:FindFirstChild('HydraStand') then
                Tween(Island.HydraStand.CFrame)
                return true
            end
        end
        for _, Chest in ipairs(Workspace:GetChildren()) do
            if Chest.Name:match('Chest') and Chest.PrimaryPart and not EventChestCache[Chest.PrimaryPart.CFrame] then
                EventChestCache[Chest.PrimaryPart.CFrame] = true
                Tween(Chest.PrimaryPart.CFrame)
                return true
            end
        end
        return false
    end
    local function FarmAutoSea2()
        if Sea1 and LocalPlayer.PlayerStats.lvl.Value >= 2250 and LocalPlayer.PlayerStats.SecondSeaProgression.Value ~= 'Yes' then
            if GetMaterial('Map') <= 0 then
                if not FarmNamed({'Seasoned Fishman [Lv. 2200]'}) then TouchNpc('Traveler') end
            else
                TouchNpc('Traveler')
            end
        end
    end
    local function FarmAutoSea3()
        if LocalPlayer.PlayerStats.lvl.Value < 4000 or Sea3 then return end
        if GetMaterial("Kraken's Cache") > 0 then
            TouchNpc('The Squid')
        elseif FarmNamed({'Tentacle'}) then
        elseif GetMaterial('Heart of Sea') <= 0 then
            if GetMaterial("Sea King's Blood") <= 0 then
                if not FarmNamed({'SeaKing', 'HydraSeaKing'}) then CollectSeaChest() end
            elseif GetMaterial('Log') < 50 then
                TouchNpc('Elite Pirate')
            else
                TouchNpc('Jack Stones')
            end
        end
    end
    ...
    Tabs.General:Dropdown({Title = 'Skills', Values = {'Z', 'X', 'C', 'V', 'B', 'E'}, Value = {'Z', 'X', 'C', 'V'}, Multi = true, Callback = function(Value)
        ...
        for Key, Enabled in pairs(Value or {}) do
            if type(Key) == 'number' then SelectedSkills[#SelectedSkills + 1] = Enabled elseif Enabled then SelectedSkills[#SelectedSkills + 1] = Key end
        end
    end})
    Tabs.General:Toggle({Title = 'Auto Farm Level', Value = false, Callback = function(Value) Flags.AutoLevel = Value end})
    Tabs.General:Toggle({Title = 'Auto Farm Nearest Mob', Value = false, Callback = function(Value) Flags.NearestMob = Value end})
    Tabs.General:Toggle({Title = 'Auto Sea 2', Value = false, Callback = function(Value) Flags.AutoSea2 = Value end})
    Tabs.General:Toggle({Title = 'Auto Sea 3', Value = false, Callback = function(Value) Flags.AutoSea3 = Value end})
    for Name, Targets in pairs(BossTargets) do
        local Tab = (Name == 'Abyssal Tyrant' or Name == 'Chaos Kraken' or Name == 'Deepsea Crusher' or Name == 'Third Sea Dragon') and Tabs.Sea or Tabs.Boss
    ...
    if Sea3 then Tabs.Travel:Button({Title = 'Current Sea 3', Callback = function() end}) end
    MakeStopTweenButton()
    task.spawn(function()
        while task.wait() do
            if not LeafStream(Flags.AutoLevel) then continue end
            pcall(function()
                local Quest = QuestData()
                if not Quest then return end
                if tostring(LocalPlayer.CurrentQuest.Value) ~= Quest.Title then
                    AcceptQuest(Quest)
                    return
                end
                if not FarmNamed({Quest.Mob}) and Quest.NPCCFrame then
                    Tween(Quest.NPCCFrame * CFrame.new(0, 60, 0))
                end
            end)
        end
    end)
    task.spawn(function()
        while task.wait() do
            if not LeafStream(Flags.NearestMob) then continue end
            pcall(function()
                MoveAttack(FindNearestMob())
            end)
        end
    end)
    task.spawn(function()
        while task.wait() do
            LeafStreamDelay(0.25)
            pcall(function()
                if Flags.AutoSea2 then FarmAutoSea2() end
    ...
    if Tabs.Info and Tabs.Info.Select then Tabs.Info:Select() end
    task.defer(function()
        Library:Notify({Title = 'Leaf Hub', Content = 'Script loaded successfully.', Icon = 'solar:check-circle-bold'})
    end)
end
...
local function AddGloss(Object)
    ...
    AddGradient(Gloss, {
        ...
        ColorSequenceKeypoint.new(1, Color3.fromRGB(180, 255, 215)),
...
local function MakeRow(Parent)
    ...
    local Stroke = AddStroke(Frame, Color3.fromRGB(65, 215, 120), 2)
    AddGradient(Frame, {
        ColorSequenceKeypoint.new(0, Color3.fromRGB(110, 255, 145)),
        ColorSequenceKeypoint.new(0.48, Color3.fromRGB(15, 30, 20)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 235, 160)),
    ...
    local Icon = New('ImageLabel', {
        ...
        ImageColor3 = Color3.fromRGB(65, 215, 120),
        BackgroundTransparency = 1,
...
local function SetRow(Row, Text, Done)
    ...
    local MainColor = Success and Color3.fromRGB(40, 100, 60) or Color3.fromRGB(15, 30, 20)
    local AccentColor = Success and Color3.fromRGB(150, 255, 180) or Fail and Color3.fromRGB(255, 90, 90) or Color3.fromRGB(65, 215, 120)
...
local function MakeButton(Parent, Text, MainColor, AccentColor)
    ...
    AddGradient(Button, {
        ...
        ColorSequenceKeypoint.new(1, Color3.fromRGB(150, 255, 110)),
    }, 12)
    AddGloss(Button)
...
local function MakeKeyGui()
    ...
    local OldGui = GuiParent:FindFirstChild('LeafGui')
    if OldGui then
        OldGui:Destroy()
    end
    local PlayerGui = LocalPlayer:FindFirstChild('PlayerGui')
    if PlayerGui and PlayerGui:FindFirstChild('LeafGui') then
        PlayerGui.LeafGui:Destroy()
    end
    ...
    local Bg = New('ImageLabel', {
        Name = 'Canvas',
        Image = 'rbxassetid://75309997803487',
        BackgroundTransparency = 1,
    ...
    New('Frame', {
        BackgroundColor3 = Color3.fromRGB(5, 14, 10),
    ...
    local Title = MakeText(Main, 'Leaf Hub', UDim2.new(0.5, 0, -0.28, 0), UDim2.new(1, 0, 0.45, 0))
    AddGradient(Title, {
        ColorSequenceKeypoint.new(0, Color3.fromRGB(150, 255, 110)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(65, 215, 120)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 235, 160)),
    })
    ...
    local Copy, CopyLabel, CopyStroke = MakeButton(Buttons, 'Get Link', Color3.fromRGB(15, 30, 20), Color3.fromRGB(0, 235, 160))
    local Check, CheckLabel, CheckStroke = MakeButton(Buttons, 'Check', Color3.fromRGB(65, 215, 120), Color3.fromRGB(150, 255, 110))
...
local function StartKeyGui()
    ...
    Ui.Copy.Activated:Connect(function()
        SetClip('https://leafhub.vercel.app/getkey?hwid=' .. Uid)
    ...
    local function RunCheck()
        ...
        if Get('https://leafhub.vercel.app/check?id=' .. Uid) then
...
if Get('https://github.com/LeafHubAcademy/LeafHub/raw/refs/heads/main/keyless.json') or Get('https://leafhub.vercel.app/check?id=' .. Uid) then
    LoadLeaf()
else
    StartKeyGui()
end
